...
type ScannerCandidate = { id: string; url: string; title: string; rawText?: string; askCents: number | null; locationText: string; distanceMiles: number | null; imageMetadata: ImageMetadata[]; updatedAt: string; reviewPriority: { score: number; posture: string; risk_level: string; valuation_status: string; factors: string[] }; analysis: { identification?: { brand?: string; model?: string; reference?: string; confidence?: number }; imageClassifications?: ImageClassification[]; recommendation?: string; rationale?: string; questions?: string[]; riskSignals?: string[]; conditionSignals?: string[]; completenessAssessment?: string[]; valuationObservations?: string[]; missingInformation?: string[] } };
...
function CandidateDossier({ candidate, onClose }: { candidate: ScannerCandidate | null; onClose: () => void }) {
  ...
  const [dealerAsk, setDealerAsk] = useState(0); const [privateAsk, setPrivateAsk] = useState(0); const [clearing, setClearing] = useState(0); const [haircut, setHaircut] = useState(10);
  const [valuation, setValuation] = useState<{ comparable_median: number; quick_liquidation_value: number; maximum_purchase_price: number; expected_purchase_alpha: number } | null>(null);
  const [valuing, setValuing] = useState(false); const [valuationError, setValuationError] = useState('');
  useEffect(() => { setBrand(identity?.brand || ''); setModel(identity?.model || ''); setReference(identity?.reference || ''); setValuation(null); setValuationError(''); }, [candidate?.id, identity?.brand, identity?.model, identity?.reference]);
  const classify = (index: number) => candidate?.analysis.imageClassifications?.find((item) => item.imageIndex === index);
  const calculate = async () => { setValuing(true); setValuationError(''); try { const response = await authorizedFetch('/api/valuation', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ dealer_ask: dealerAsk, private_ask: privateAsk, clearing_estimate: clearing, qlv_haircut_percent: haircut, asking_price: Math.round((candidate?.askCents || 0) / 100), transaction_cost: 0, risk_penalty: 0, minimum_alpha: 200 }) }); const data = await response.json(); if (!response.ok) throw new Error(data?.detail?.error || 'Valuation service unavailable.'); setValuation(data); } catch (error) { setValuationError(error instanceof Error ? error.message : 'Valuation service unavailable.'); } finally { setValuing(false); } };
  ...
  return <Sheet open={Boolean(candidate)} onOpenChange={(open) => { if (!open) onClose(); }}><SheetContent className="w-full overflow-y-auto p-0 sm:max-w-4xl"><SheetHeader className="border-b bg-[#07101d] p-6 pr-14 text-white"><p className="text-xs font-semibold uppercase tracking-[.18em] text-cyan-300">Candidate evidence dossier</p><SheetTitle className="mt-1 text-2xl text-white">{identity?.brand || candidate?.title || 'Watch candidate'} {identity?.model || ''}</SheetTitle><SheetDescription className="text-slate-300">AI evidence review · priority {candidate?.reviewPriority.score ?? 0} · {candidate?.reviewPriority.risk_level || 'unknown'} risk</SheetDescription></SheetHeader>{candidate && <div className="space-y-6 p-5 md:p-6">
    ...
    <section className="rounded-2xl border bg-slate-50 p-4"><h3 className="font-semibold">Comparable-based valuation</h3><p className="mt-1 text-xs leading-5 text-slate-500">Enter human-sourced comps. The backend uses the median, applies the liquidity haircut, and enforces a $200 minimum alpha.</p><div className="mt-3 grid grid-cols-2 gap-3 md:grid-cols-4"><CompactMoney label="Dealer ask" value={dealerAsk} onChange={setDealerAsk} /><CompactMoney label="Private ask" value={privateAsk} onChange={setPrivateAsk} /><CompactMoney label="Clearing estimate" value={clearing} onChange={setClearing} /><label className="text-xs text-slate-500">QLV haircut %<Input className="mt-1" type="number" min="0" max="50" value={haircut} onChange={(e) => setHaircut(Number(e.target.value))} /></label></div><Button className="mt-3 bg-[#0a68a5] hover:bg-[#08598d]" disabled={valuing || dealerAsk <= 0 || privateAsk <= 0 || clearing <= 0} onClick={() => void calculate()}>{valuing ? <Loader2 className="animate-spin" /> : <Gauge />} Calculate on backend</Button>{valuationError && <p className="mt-2 text-sm text-rose-700">{valuationError}</p>}{valuation && <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-4">{[['Comp median', valuation.comparable_median], ['Quick liquidation', valuation.quick_liquidation_value], ['Max purchase', valuation.maximum_purchase_price], ['Alpha at ask', valuation.expected_purchase_alpha]].map(([label, value]) => <div key={label} className="rounded-xl bg-white p-3"><div className="text-xs text-slate-500">{label}</div><div className="mt-1 font-semibold">{money(Number(value))}</div></div>)}</div>}</section>
