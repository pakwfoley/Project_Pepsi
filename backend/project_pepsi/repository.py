...
def save_analysis(session: Session, listing: ListingIngest, analysis: WatchAnalysis) -> ScannerCandidate:
    ...
    candidate = session.scalar(select(ScannerCandidate).where(ScannerCandidate.source == listing.source, ScannerCandidate.source_listing_id == key))
    if candidate is None:
        candidate = ScannerCandidate(source=listing.source, source_listing_id=key, url=str(listing.url), title=listing.title)
...
def save_manual_listing(session: Session, payload: ManualListingInput, source: str, confidence: float, economics: dict[str, int]) -> Listing:
    listing = Listing(url=str(payload.url), source=source, raw_text=payload.rawText, brand=payload.brand.strip(), model=payload.model.strip(), reference=payload.reference.strip().upper(), ask_cents=economics["ask"], normalization_confidence=confidence)
    session.add(listing)
    session.flush()
